...
export function MotionControls({ config, set, source, trim, time, seekTo, endpoint = 0, setEndpoint }) {
  ...
  const controls = availableMotionControls(config.effect, config);
  ...
  return <section className="motion-controls">
    ...
    {Object.keys(motion.tracks).length > active.length && <p className="hint">Tracks for other treatments are kept and resume when you return to them.</p>}
  </section>;
}
