...
2. Choose a classic starter in **Looks**, or explore **Artistic** for expressive materials and patterns. Tune **Effect**, **Color**, **Select**, and **Frame**, then use **Before / after** to compare. On compact screens, **Preview**, **Browse**, **Adjust**, and **Export** keep the main activities within reach. Expand controls for detailed editing while retaining playback. Open the gallery from Looks or Artistic for larger style cards.
3. Open **Layers** to combine up to three treatments, reorder or bypass them, and adjust their blends. **Effect**, **Color**, and **Motion** all edit the selected layer, whose name and position stay visible. **Frame** applies to the whole composition; selection, custom fonts, and color echoes belong to the main effect. Added layers use built-in fonts. Applying a look returns to the main treatment. Stacks are saved in projects and presets and work across every export type.
...
- Look previews use the same framing, added layers, and trim-relative animation as applying a look. Moving to another frame clears stale previews. Stopping or closing a batch cancels its work, and late errors cannot overwrite the current gallery.
- Forty-eight style starters. The Artistic tab curates thirty-eight across nineteen techniques and remembers the selected technique while editing. Search by name, material, or description; expand the gallery without losing the filter. Cards distinguish sample artwork from previews of your frame. Preview visible results on demand and stop a batch at any time.
...
- Two spatial techniques offer five looks: lifted paper ridges and solid adaptive patterned tiles. Harmonic field is retired from discovery, with saved-project compatibility retained. Continuous animation controls share the video and still-export renderer.
...
- A scrollable, searchable color browser shows all 41 curated palettes and every ink, including echo colors. Palettes range from two-color prints to ten-color textiles; dark mode uses neutral black surfaces. Original character-set collections remain available. **Keep selection when changing looks** lets you try looks without losing a tuned selection.
...
- `src/studio/spatial-art.js`, `contour-plate.js`: occluding relief, cached harmonic fields, bounded contour tracing, and image-adaptive tiles.
- `src/studio/effect-scale.js`: shared pattern labels, minimum sizes, and sampling rules for controls, imports, and rendering.
...
- `src/studio/ExportResult.jsx`, `ExportReview.jsx`: finished-file playback, metadata, downloads, and sharing.
...
See [Spatial studies](docs/SPATIAL-STUDIES.md) for the new effects, focused cleanup, and verification. Render their contact sheets with `node scripts/check-artistic.mjs /path/to/review --spatial --preview-only`.

See [Palette and tile refinement](docs/PALETTE-AND-TILE-REFINEMENT.md) for solid subdivision, the full color browser, palette curation, and neutral black appearance.
