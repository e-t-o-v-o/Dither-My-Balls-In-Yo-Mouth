...
The internal HTTP preview covered the live compatibility path and GIF. The subsequent HTTPS deploy preview enabled native browser WebCodecs: both High (VP8/Opus) and Maximum (VP9/Opus) produced independently decoded 1920 × 1080 downloads with exactly 60 video frames for the two-second trim. Auto correctly selected WebM in this Chromium environment. The files have a 2.020-second container duration because the final Opus packet extends 20 ms beyond the video; frame counts are exact, while encoded audio packet padding can extend container duration. MP4/H.264/AAC was exercised with the native Node adapters described above. On-device Safari testing, camera permission flows, and sustained large 4K workloads remain necessary before claiming certification for those environments.

CI runs the automated suite, real codec integration check, GIF worker smoke check, production build, and dependency audit. The codec check keeps Node alive while native background callbacks finish and fails after a bounded timeout. The point-in-time dependency audit is stored in [dependency-audit.json](dependency-audit.json).
